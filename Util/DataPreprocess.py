...
class modified_data_loader(Dataset):
    def __init__(self, samples, labels, win_size=128, transform=None):
        self.samples = samples
        self.labels = labels
    ...
    def __getitem__(self, index):
        ...
        modified_signal = modified_signal.reshape(self.win_size,1,3)
...
def modified_load_train_valid_test(train_x, train_y, valid_x, valid_y, test_x, test_y, batch_size=64, win_size=128, drop_last=True):
    train_x_swap = np.swapaxes(train_x,1,3)
    valid_x_swap = np.swapaxes(valid_x,1,3)
    test_x_swap = np.swapaxes(test_x,1,3)
    
    train_set = modified_data_loader(train_x_swap, train_y, win_size)
    valid_set = modified_data_loader(valid_x_swap, valid_y, win_size)
    test_set = modified_data_loader(test_x_swap, test_y, win_size)
