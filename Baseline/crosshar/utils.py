...
def partition_and_reshape(data, labels, label_index=0, training_rate=0.8, vali_rate=0.1, change_shape=True
    ...
    print('Total Size: %d, Train Size: %d, Vali Size: %d, Test Size: %d' % (data.shape[0], label_train.shape[0], label_vali.shape[0], label_test.shape[0]))
    return data_train, label_train, data_vali, label_vali, data_test, label_test


def prepare_simple_dataset(data, labels, training_rate=0.8):
    arr = np.arange(data.shape[0])
    np.random.shuffle(arr)
    data = data[arr]
    labels = labels[arr]
...
def handle_argv(target, config_train, prefix):
    ...
    parser.add_argument('-d', '--dataset', type=str, default='uci', help='Dataset name', choices=['hhar', 'motion', 'uci', 'shoaib', 'dsads_s', 'opportunity_s', 'pamap2_s', 'wisdm_s'])
    parser.add_argument('-td', '--target_dataset', type=str, default='uci', help='Dataset name', choices=['hhar', 'motion', 'uci', 'shoaib', 'dsads_t', 'opportunity_t', 'pamap2_t', 'wisdm_t'])
